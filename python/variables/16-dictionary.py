...
# 💥 딕셔너리 만들 때 주의할 사항
# 딕셔너리에서 Key는 고유한 값이므로 중복되는 Key 값을 설정하면 하나를 제외한 나머지가 모두 무시됨
a = {1:'a', 1:'b'}
print(a) # {1: 'b'}

# Key에는 리스트를 쓸 수 없다(오류 발생).
# 튜플은 Key로 쓸 수 있다. 
# 딕셔너리의 Key로 쓸 수 있는지 없는 지는 Key가 변하는 값인지 아닌지에 달려있다.
# 리스트는 그 값이 변할 수 있기 때문에 Key로 쓸 수 없다. 
# 단, Value에는 변하는 값이든 변하지 않는 값이든 상관없이 아무 값이나 넣을 수 있다. 리스트, 튜플 모두 가능