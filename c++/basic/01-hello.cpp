...
{
    // 표준 라이브러리의 출력 연산자(output operator) <<는 왼쪽 우선 결합성(left-associative)을 지니고 있다.
    // std::로 시작하는 코드는 해당 이름이 std라는 namespace에 속해 있음을 나타낸다.
    // (std::cout은 '네임스페이스 std 범위에 있는 cout이라는 이름'을 의미)
    // endl은 줄 바꿈 역할을 한다.
